...
function lib:init(loader_name,available_games_url)
    if table.find({6808416928},tonumber(game.PlaceId)) then warn("Don't loading the loader to avoid game ban."); return
        ...
        if type(available_games_url) == "string" then
            ...
            if r.Body ~= "404: Not Found" then 
                ...
                if json[tostring(game.PlaceId)] then
                    ...
                    --if json[tostring(game.PlaceId)]["lobby"] then lobby = json[tostring(game.PlaceId)]["lobby"] end -- cosmetic only. (get lobby icon + name)
                    --if json[tostring(game.PlaceId)]["game"] then game = json[tostring(game.PlaceId)]["game"] end -- cosmetic only. (get script author)
    ...
    do -- Root.General setup
        spawn(function()
            ...
            if json ~= nil then
                --if json[tostring(game.PlaceId)] then
                --    if json[tostring(game.PlaceId)].lobby ~= nil then
                --        if type(json[tostring(game.PlaceId)].lobby) == "string" then
                --            Text_2.Text = game:GetService("MarketplaceService"):GetProductInfo(json[tostring(game.PlaceId)].lobby).Name 
                --        end 
                --    end
                --end
...
getgenv().ngstloader = lib:init("nGSTLoader","https://raw.githubusercontent.com/GameSTALkER/synapse.luas/main/data/games.json")

getgenv().ngstloader:CustomScript("AnimHub","https://raw.githubusercontent.com/GameSTALkER/synapse.luas/scripts/menu.scripts/AnimHub.lua")
getgenv().ngstloader:CustomScript("SSpy","https://raw.githubusercontent.com/GameSTALkER/synapse.luas/scripts/menu.scripts/SimpleSpy.lua")
getgenv().ngstloader:CustomScript("ESP","https://raw.githubusercontent.com/GameSTALkER/synapse.luas/scripts/menu.scripts/ESP.lua")
getgenv().ngstloader:CustomScript("InfY","https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source")
getgenv().ngstloader:CustomScript("BackDoor","https://raw.githubusercontent.com/GameSTALkER/synapse.luas/scripts/menu.scripts/backdoor.lua")
