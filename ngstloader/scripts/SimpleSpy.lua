...
function SimpleSpy:ArgsToString(method, args)
    ...
    return method .. "(unpack(" .. v2v({args = args}) .. "))"
end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in pairs(t) do -- iterates over table
        ...
        if rawequal(k, t) then -- checks if the table being iterated over is being used as an index within itself (yay, lua)
            bottomstr = bottomstr .. tostring(n) .. tostring(path) .. "[" .. tostring(n) .. tostring(path) .. "]" .. " = " .. (rawequal(v, k) and tostring(n) .. tostring(path) or v2s(v, l, p, n, vtv, k, t, path .. "[" .. tostring(n) .. tostring(path) .. "]", tables))
            size = size - 1
        end
        ...
        s = s .. string.rep("", l) .. "[" .. k2s(k, l, p, n, vtv, k, t, path .. currentPath, tables, tI) .. "] = " .. v2s(v, l, p, n, vtv, k, t, path .. currentPath, tables, tI) .. ","
    end
...
newButton(
    "Copy PlaceId",
    function() return "Click to copy PlaceId" end,
    function()
